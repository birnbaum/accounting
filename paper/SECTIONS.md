...
| §7 | Conclusion | `sec:future` | Tight wrap + outlook: `Beyond carbon` (water, bounding reconciliation error) and `Toward a real-time signal` (per-component state estimation, Kalman lifted from energy, `sharma2024`). Real-time para closes the paper. |
