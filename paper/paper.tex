...
% TODO: full Introduction + Abstract framing is deferred until §8 (Evaluation/case study)
% results land. The thesis sentence, contributions list, and abstract will be tightened
% once we know what the empirical story actually supports. §2--§6 math and content are
% committed; the framing wrapper around them is the moving piece.
...
Bashir's critique exposes one weakness of \textsc{sci}: embodied-carbon mis-allocation
biasing scheduling. We argue there is a second, structurally distinct weakness: every
member of the \textsc{sci} family is constructed without reference to the top-down
provider report it must reconcile with. The next section grounds this critique in what
providers actually publish today.
...
% NOTE: high-level framing (one-sentence thesis at the open, properties-and-implications
% paragraph at the close, choice of "solves the framework / instantiates the
% parameterization" emphasis) is deferred until the case-study results land in §8.
% The math below is committed; the framing wrapper around it will be tightened once
% we know what the empirical story supports.
...
\paragraph{Why the sunk-carbon fallacy does not arise.}
In \textsc{sci}, the embodied term $M$ is amortised per server and enters
\emph{per unit of work} $R$, so it shrinks as inefficient hardware ages
(\S\ref{sec:background}). In rSCI, embodied carbon lives in
$\Delta^{\text{S3}}$ and is allocated \emph{per unit of energy}:
$w_{i,s,r}\Delta^{\text{S3}}_{s,r} \propto E_{i,s,r}$.
A workload run on older, less efficient hardware draws more energy, takes a larger
share of the residual, and reports a \emph{higher} rSCI --- the opposite of the
\textsc{sci} pathology. Routing to legacy hardware therefore stops looking green.
...
any level of data fidelity. The asks in \S\ref{sec:asks} target the second axis.
